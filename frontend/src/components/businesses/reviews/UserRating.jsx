...
const UserRating = () => {
  ...
  const { data, isSuccess } = useQuery(['drafts_for_', business_name], () =>
  ...
  const { mutate } = useMutation((draft_id) =>
    draftService.deleteDraft(business_name, draft_id)
  );
            ...
            {data.drafts.map((review, idx) => (
              <>
                ...
                <div className="mt-5 flex gap-7 items-center">
                  <button className="border border-[#c8c9ca] px-[16px] py-[8px] rounded text-black font-medium hover:bg-gray-200 ease-in-out duration-700">
                    Finish My Review
                  </button>
                  <Link
