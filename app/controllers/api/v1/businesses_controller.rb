  ...
  module V1
    class BusinessesController < ApplicationController
      ...
      def latest
        ...
        render :latest, status: :ok if stale?(last_modified: @latest_businesses.last.updated_at)
      end
      ...
      def search(search_category, country, state)
        @businesses = if !country.nil?
                        Business.search_by_category_and_country(search_category, country)
                      elsif !state.nil?
                        Business.search_by_category_and_country_and_state(search_category, country, state)
                      else
      ...
      def edit
        ...
        if @business.update(business_params_edit) && stale?(last_modified: @business.updated_at, public: true)
